...
def test_org_sync_single_new(db, dj_account_objects):
    ...
    assert models.OrganizationUser.objects.filter(org=new_org.id, user=dj_account_objects.user.id).exists()
...
def test_org_sync_multiple(db, dj_account_objects):
    ...
    org_names = set([orguser.org.name for orguser in dj_account_objects.user.org_set.all()])
    assert org_names == set(["org3-test", "org4-test"])


def test_orguser(db, dj_account_objects):
    orguser = models.OrganizationUser.objects.filter(org=dj_account_objects.org, user=dj_account_objects.user).first()
